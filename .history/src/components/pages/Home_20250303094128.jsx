...
const Home = ({ filteredProducts, searchTerm, highlightText,addToWishList, addToCart}) => {
  const [filter, setProducts] = useState(filteredProducts);
  const dispatch = useDispatch();

  const groupByCategory = (filter) => {
    return filter.reduce((acc, product) => {
  ...
  const groupedProducts = groupByCategory(filter);
...
const styles = {
  ...
  productsGrid: { display: "flex", flexWrap: "wrap", gap: "10px" ,border:"1px solid red", padding:"10px"},
  productBox: {
    width: "250px",
    height: "250px",
    backgroundColor: "",
