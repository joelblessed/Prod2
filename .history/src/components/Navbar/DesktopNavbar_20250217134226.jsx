...
function DesktopNavbar({
  ...
  return (
    <nav className="desktop-navbar">
      ...
      />
      <button onClick={search} style={{position:"absolute", left:"800px"}}>
        🔍
      </button>
    </div>
        </li>
        ...
        <li href="#" style={{ width: "auto" }}>
          ...
          <NavLink to="/category">
            <select
              ...
              onChange={(e) => setSearchTerm(e.target.value)}
            >
              <option value=""onClick={search}>Category</option>
              {categories.map((category,index) => (
               <option key={index} value={category} onClick={search}>{category}</option>
              ))}
