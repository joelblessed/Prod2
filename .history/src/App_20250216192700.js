...
function App() {
  ...
  useEffect(() => {
    fetch("/db.json") // if using create-react-app, place db.json in public/
      .then((response) => response.json())
      .then((data) => {
        // Check if using json-server, adjust if needed
        // If using json-server and hitting http://localhost:3000/products,
        // then data might be an array directly.
  ...
  const handleSearch = () => {
    const filtered = products.filter((product) => {
      // Check if product category includes search text (case-insensitive)
      const categoryMatch = searchCategory
        ? product.category.toLowerCase().includes(searchTer.toLowerCase())
        : true;

      // Check if product name includes search text (case-insensitive)
      const productNameMatch = searchProductName
        ? product.name.toLowerCase().includes(searchTerm.toLowerCase())
        : true;

      // Check if any brand name in the product's brand array includes the search text
      const brandMatch = searchBrandName
