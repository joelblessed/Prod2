...
function App() {
  ...
  const handleLogin = (userData) => {
    setUser(localStorage.getItem("use"););
  ...
  const mergeCartWithServer = async (userId) => {
    try {
      const res = await fetch(`/cart?userId=${userId}`);
      const serverCart = await res.json();
      
      const mergedCart = [...serverCart[0]?.items || [], ...cart];
    ...
    <AuthProvider>
      ...
      <Router>
        ...
        <Routes>
          ...
          <Route path="/signIN" element={<SignIN api={api}  handleLogin={handleLogin}/>} />
