...
// //////////////////////////
  ...
  // Load cart from localStorage before login
  ...
  const handleLogin = async (userData) => {
    setUser(userData);
    await mergeCartWithServer(userData.id);
  };

  // Merge local cart with server cart after sign-in
  const mergeCartWithServer = async (userId) => {
    try {
      const res = await fetch("/cart");
      const serverCartData = await res.json();

      // Find user's cart from the response
      const userCart = serverCartData.find((c) => c.userId === userId) || { items: [] };

      // Merge local and server cart (remove duplicates)
      const mergedCart = [...userCart.items, ...cart].reduce((acc, item) => {
  ...
  // Add product to cart (before or after login)
