...
function App() {
 ...
 useEffect(() => {
  fetch(`${api}/products`) // If using create-react-app, put db.json in the public folder
    .then((response) => response.json())
    .then((data) => {
      // Adjust if your data is directly an array or nested under a key (e.g., data.products)
...
const handleSearch = () => {
  ...
  const filtered = products.filter((product) => {
    ...
    const categorySelect = selectedCategory ?product.category == selectedCategory : true
    // Check if any brand name in the brand array matches the query
    const brandMatch = product.brand.some((b) =>
      b.name.toLowerCase().includes(query)
    );

    // Use OR logic: match if any field contains the query
    return categoryMatch || productNameMatch || brandMatch || categorySelect;
  });
